...
class RenovasiRumahFragment : Fragment() {
    ...
    // === Tambahan: guard supaya event dari fragment lain tidak ikut dieksekusi
    ...
    companion object {
        private const val REQUEST_IMAGE_PICK = 2001
        private const val ARG_EDIT_DOCUMENT = "arg_edit_document"
        fun newInstanceForEdit(document: RenovasiRumah): RenovasiRumahFragment {
            val f = RenovasiRumahFragment()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState); setHasOptionsMenu(false)
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        val layoutId = resources.getIdentifier("fragment_renovasi_rumah", "layout", requireContext().packageName)
        val view = inflater.inflate(if (layoutId != 0) layoutId else R.layout.fragment_renovasi_rumah, container, false)
        ...
        etDeskripsiRenovasi = view.findViewById(resources.getIdentifier("etDeskripsiRenovasi", "id", requireContext().packageName))
        ...
        arguments?.getSerializable(ARG_EDIT_DOCUMENT)?.let {
            editingDocument = it as RenovasiRumah
    ...
    private fun setupObservers() {
        documentViewModel.uploadResult.observe(viewLifecycleOwner) { result ->
            // >>> Guard: hanya proses kalau upload ini dipicu oleh fragment ini
            if (!awaitingUpload) return@observe

            if (result.isSuccess) {
                // Setelah upload selesai, beralih ke tahap simpan
    ...
    private fun populateFields(d: RenovasiRumah) {
        etNama.setText(d.nama)
        etAlamat.setText(d.alamat)
        etNoTelepon.setText(d.noTelepon)
        etDeskripsiRenovasi.setText(d.deskripsiRenovasi)
        d.attachments.forEach { (n, u) -> selectedImages.add(DocumentImage(n, url = u)) }
    ...
    private fun saveDocument() {
        ...
        // tandai bahwa proses upload-simpan ini milik fragment ini
        awaitingUpload = true
        awaitingSave = false
        ...
        if (locals.isNotEmpty()) {
            documentViewModel.uploadFiles(
                (requireContext()), locals.map { it.uri!! },
                Constants.DOC_TYPE_RENOVASI_RUMAH,
                locals.map { it.name }
            )
        } else {
            // tidak ada file → langsung simpan (tetap pakai guard save)
            saveDocumentWithImages(emptyList())
        }
    }

    private fun saveDocumentWithImages(uploadedUrls: List<String>) {
        // saat akan menyimpan, aktifkan guard save
        awaitingSave = true
        ...
        val doc = RenovasiRumah(
            id = d?.id ?: "",
            uniqueCode = d?.uniqueCode ?: CodeGenerator.generateCodeForRenovasiRumah(),
        ...
        documentViewModel.saveRenovasiRumah(doc)
    }

    private fun clearForm() { /* tidak digunakan saat edit */ }

    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        if (requestCode == REQUEST_IMAGE_PICK && resultCode == Activity.RESULT_OK) {
            fun addOne(uri: Uri) {
                selectedImages.add(DocumentImage("Document_${System.currentTimeMillis()}", uri))
            }
            ...
            if (clip != null && clip.itemCount > 0) {
                ...
                addOne(uri); imageAdapter.notifyItemInserted(selectedImages.size - 1)
            }
        }
    }
}
