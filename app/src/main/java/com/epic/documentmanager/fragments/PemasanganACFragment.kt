...
class PemasanganACFragment : Fragment() {
    ...
    private lateinit var etJenisTipeAC: EditText   // input Jenis/Tipe AC
    ...
    companion object {
        private const val REQUEST_IMAGE_PICK = 3001
        private const val ARG_EDIT_DOCUMENT = "arg_edit_document"
        fun newInstanceForEdit(document: PemasanganAC): PemasanganACFragment {
            val f = PemasanganACFragment()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        val layoutId = resources.getIdentifier("fragment_pemasangan_ac", "layout", requireContext().packageName)
        val view = inflater.inflate(if (layoutId != 0) layoutId else R.layout.fragment_pemasangan_ac, container, false)
        ...
        // Bind id untuk Jenis/Tipe AC (fallback jika id beda)
        val jenisId = resources.getIdentifier("etJenisTipeAC", "id", requireContext().packageName)
            .takeIf { it != 0 } ?: resources.getIdentifier("etJenisAC", "id", requireContext().packageName)
        etJenisTipeAC = view.findViewById(jenisId)
        ...
        arguments?.getSerializable(ARG_EDIT_DOCUMENT)?.let { editingDocument = it as PemasanganAC; populateFields(editingDocument!!) }
        return view
    }
    ...
    private fun populateFields(d: PemasanganAC) {
        etNama.setText(d.nama)
        etAlamat.setText(d.alamat)
        etNoTelepon.setText(d.noTelepon)
        etJumlahUnit.setText(d.jumlahUnit.toString())
        // gunakan nama field di model: jenisTipeAC
        etJenisTipeAC.setText(d.jenisTipeAC)
    ...
    private fun saveDocument() {
        ...
        if (locals.isNotEmpty()) documentViewModel.uploadFiles(requireContext(), locals.map { it.uri!! }, Constants.DOC_TYPE_PEMASANGAN_AC, locals.map { it.name })
    ...
    private fun saveDocumentWithImages(uploadedUrls: List<String>) {
        ...
        val doc = PemasanganAC(
            id = d?.id ?: "",
            uniqueCode = d?.uniqueCode ?: CodeGenerator.generateCodeForPemasanganAC(),
            nama = etNama.text.toString().trim(),
            alamat = etAlamat.text.toString().trim(),
            noTelepon = etNoTelepon.text.toString().trim(),
            // gunakan nama parameter yang ada di model: jenisTipeAC
            jenisTipeAC = etJenisTipeAC.text.toString().trim(),
        ...
        documentViewModel.savePemasanganAC(doc)
    }
